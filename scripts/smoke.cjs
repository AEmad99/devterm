...
if (typeof electron === 'string') {
  ...
  // Restricted Windows runners can terminate Electron's bootstrap with
  // STATUS_BREAKPOINT before the app starts. These flags apply only to this
  // headless dependency probe; DevTerm's BrowserWindow sandbox is unaffected.
  const electronArgs = process.platform === 'win32' ? ['--no-sandbox', '--disable-breakpad'] : []
  const result = spawnSync(electron, [...electronArgs, __filename], {
    stdio: 'inherit',
    env: process.env
  })
  if (result.error) {
