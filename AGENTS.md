...
| Command blocks / input editor | `src/renderer/lib/{command-blocks,shell-highlight,autosuggest}.ts`, `CommandInput.tsx` |
...
- **Command input editor + gutters** (`lib/command-blocks.ts`, `CommandInput.tsx`): when OSC 133 A/B hooks are healthy, a one-line highlighted editor appears for the next command (Enter sends keystrokes to the shell). Completed A-then-B commands get a faint gutter (Copy / Ask agent / Comment). Comments are local to the pane and appended the next time that pane’s agent is prompted. Missing hooks hide the editor. No OSC 133 C/D exit-code coloring.
...
- **ProxyJump is capped at two extra hops** (three including the target). Command blocks are A/B gutters plus an optional input editor; OSC 133 C/D exit-code coloring and a Warp-style block canvas are not shipped. No programmable app CLI/socket API, no inline images/sixel, no OSC 9/99 attention protocol.
...
- Open GitHub issues (`AEmad99/devterm`): **#1** command syntax highlighting — highlighting applies to the optional OSC 133 input editor and CodeMirror files; the raw PTY stream stays the shell’s. **#2** preview + annotate MVP is in-tree (localhost / forwarded port / local folder, comments, MCP tools); leftover polish (port detect, richer drawing) can be follow-up tickets.
