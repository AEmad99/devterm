...
3. Spawns the agent in a node-pty with the binary + args its launch layer picked:
   ...
   - **opencode** (sst/opencode): `opencode --pure <temp-dir>`, with `OPENCODE_CONFIG=<temp-dir>/opencode.json` in the env. The config has a `mcp.devterm` remote entry pointing at the bridge, every built-in tool disabled (so all host work must go through the `devterm_*` MCP tools), `autoupdate: false`, `share: 'disabled'`, and the bundled control server pinned to `127.0.0.1` with its `port` omitted (an explicit `port: 0` is rejected by opencode's schema — `exclusiveMinimum 0` — and aborts launch; omitting it gets the same OS-assigned port). opencode only ever *merges* config layers (no "ignore global config" switch, and a merge can't delete a key), so to keep the operator's own `~/.config/opencode` from leaking into the air-gapped session DevTerm subtracts capabilities rather than isolating the dir: it reads the operator's mcp server names and writes `enabled: false` for each (the per-session config has higher precedence, so even `type: local` servers that spawn local processes resolve as disabled and never start), and passes `--pure` to drop external plugins. The operator's real config dir is left loading on purpose — it carries their `provider`/`model`/auth and opencode's already-installed plugin runtime, so the session starts instantly and works offline; relocating the config dir would instead force opencode to re-install its plugin runtime into an empty dir on every launch.
