...
- Tab labels compress long agent/shell activity (`lib/tab-label.ts`); busy tabs are width-capped. The first-run checklist is a full-width row of three cards above the panes (local terminal, save/import a connection, open Agent). Finished steps stay marked done, and each open card opens that surface. The row hides after all three or Dismiss — settings import must not resurrect it. Picking a theme is not required to finish it.
...
- **Command blocks** (`lib/command-blocks.ts`): OSC 133 tracking and per-pane comments remain in the library. The pane does not mount a command header or gutter bars — those covered the live prompt and stopped typed characters from painting while output was streaming. DevTerm's own prompt hooks emit A/B only. Comments already saved are appended the next time that pane’s agent is prompted. There is no bottom command input strip.
...
- Voice dictation: renderer-only Whisper (`src/renderer/lib/stt/*`), WebGPU→WASM fallback, push-to-talk (Ctrl/Cmd+Shift+M). Model weights are not in the installer; the first dictation downloads the chosen Whisper model and caches it in `persist:browser`. `ort/*.wasm` (the runtime, not the model) must stay `asarUnpack`ed, once. Worker crash recovery discards stale ready messages.
...
- **ProxyJump is capped at two extra hops** (three including the target). The terminal does not mount a command header or highlight bars. A Warp-style block canvas is not shipped. No programmable app CLI/socket API, no inline images/sixel, no OSC 9/99 attention protocol.
