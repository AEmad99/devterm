...
- DevTerm Agent provider/model preferences, authenticated-provider status, ordered rate-limit fallbacks, and local performance telemetry live in Settings. Resumable conversations use `userData/agent-sessions/`; executable third-party extensions remain disabled, while explicitly selected instruction-only skill files are SHA-256 pinned before every launch and still act only through MCP policy.
...
- SSH reconnect restores the human shell channel automatically. When `remoteDetachedSessions` is enabled, POSIX shells attach to a stable tmux session if tmux is available; the normal-shell fallback still reopens after transport recovery.
...
- `userData`: `connections.json` (secret fields safeStorage-encrypted, incl. bastion `jump`), `workspaces.json`, `snippets.json`, `approval-rules.json`, `known_hosts.json` (0o600), `transfers.json`, `browser-zoom.json`, `bridge-activity.jsonl`, `settings.json` (renderer mirror), `agent-sessions/` (optional resumable transcripts), `search/<sessionId>.jsonl` (optional), `quick-connect.json`, `Downloads/`.
