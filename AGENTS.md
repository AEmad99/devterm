...
3. Spawns the agent in a node-pty with the binary + args its launch layer picked:
   ...
   - **opencode** (sst/opencode): `opencode <temp-dir>`, with `OPENCODE_CONFIG=<temp-dir>/opencode.json` and `XDG_CONFIG_HOME=<temp-dir>/config-home` (an empty dir) in the env. The config has a `mcp.devterm` remote entry pointing at the bridge, every built-in tool disabled (so all host work must go through the `devterm_*` MCP tools), `autoupdate: false`, `share: 'disabled'`, and the bundled control server pinned to `127.0.0.1` with its `port` omitted (an explicit `port: 0` is rejected by opencode's schema — `exclusiveMinimum 0` — and aborts launch; omitting it gets the same OS-assigned port). Because opencode only ever *merges* config layers (no "ignore global config" switch, and a merge can't delete a key), the empty `XDG_CONFIG_HOME` severs the operator's `~/.config/opencode` — its mcp servers (incl. `type: local` ones that spawn local processes) and plugins — from the air-gapped session; only the operator's `provider`/`model` are lifted back across so their chosen model still runs. Auth lives in the data dir (`XDG_DATA_HOME`), which is left in place so authed providers keep working.
