...
- **Motion/animations are a CSS-only layer at the end of `styles.css`** (after the glass/frameless sections): a `@keyframes` set plus one `@media (prefers-reduced-motion: no-preference)` block, with a motion-token scale (`--ease-out`, `--ease-spring`, `--dur-fast`/`--dur`/`--dur-slow`) in a trailing `:root`. Rules animate **only** `opacity`/`transform` (GPU-friendly) and **never** touch the xterm viewport. Entrances (modal/palette pop-in, manager-view + staggered `.manager-row`/`.nt-choice` fade-up) rely on those elements mounting fresh; the rest are hover/focus transitions (pane focus ring, group tabs, active-tab underline, primary buttons). Reuse the tokens and keep any new motion inside the reduced-motion guard — don't hardcode durations/easings.
