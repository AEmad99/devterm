    ...
    <div className="transfers-panel">
      ...
      {items.length === 0 ? (
        <div className="transfers-empty">
          No transfers yet. Drag a file from one file pane to another, or use
          the per-row Upload / Download buttons.
        </div>
      ) : (
...
function TransferRow({
  ...
  return (
    <li className={`transfers-row status-${status}`}>
      ...
      <span className="transfers-name" title={item.direction === 'upload' ? item.localPath : item.remotePath}>
        {name}
      </span>
