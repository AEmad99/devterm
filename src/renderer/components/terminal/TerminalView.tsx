...
function TerminalView({ session }: { session: Session }) {
  ...
  useEffect(() => {
    ...
    // Seed the global search index with whatever is already in xterm's buffer
    // (scrollback + visible lines) so search works for historical output, not
    // just lines emitted after this component mounted.
    const seedSearch = () => {
      try {
        ...
        if (lines.length > 0) {
          window.devterm.search.seed(session.id, lines).catch(() => undefined)
        }
      } catch {
    ...
    term.attachCustomKeyEventHandler((e) => {
      ...
      if (!e.altKey && !e.shiftKey && ((e.ctrlKey && k === 'v') || (e.metaKey && k === 'v'))) {
        return false
      }
      ...
      if (!e.altKey && !e.shiftKey && ((e.ctrlKey && k === 'c') || (e.metaKey && k === 'c'))) {
      ...
      const id = matchHotkey(e, resolveHotkeys(useSettings.getState().keybindings))
      if (id === 'find') {
        setFindOpen(true)
        return false
      }
