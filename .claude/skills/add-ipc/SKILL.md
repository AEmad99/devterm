...
2. **`src/main/ipc/`** — add the handler in the most relevant existing module (`pty`, `ssh`, `files`, `agent`, `connections`, `context`) or a new one, following the existing `ipcMain.handle`/registration pattern. Make sure it's registered from `src/main/index.ts` if you add a new module.
