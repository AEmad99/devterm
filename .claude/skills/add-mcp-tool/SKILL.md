---
name: add-mcp-tool
description: Guided workflow for adding a new agent tool to the DevTerm MCP bridge, wired through its policy guardrail. Use when adding a capability the embedded Claude agent can call.
---

Add a new MCP tool that the embedded `claude` CLI can call over the in-process MCP server. Follow the existing patterns — read the files before editing.
...
5. Run `npm run typecheck` and, if relevant, exercise the tool via the Claude pane in `npm run dev`.
